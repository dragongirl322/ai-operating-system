**


- Objective 1: Reach $12,000 average monthly sales by end of year
    
- Objective 2: Win "Best Of Edmonds" Music Store category
    

## Success Definition

Revenue targets (run-rate)

- Q1 average: $8K/month
	- Actual $5768 / month
    
- Q2 average: $8K/month
    
- Q3 average: $10K/month
    
- Q4 average: $12K/month
    

What must be true for us to hit $12K/month

- Store awareness increases
    
- Weekly in store traffic is consistent 
    
- Repeat customers increase month-over-month
    
- Core titles stay in stock (no "sold-out staples")
    
- Inventory cash isn't trapped in dead stock
    

## Operating Cadence

Every transaction

- Ask every customer to join Groove Society
- If customer is in Groove Society, associate the sale to their account


2x a week

- Capture at least 1 photo or short clip for social and post


Every week

- Generate Reorder List + place orders: Tre
    
- Confirm micro-events + monthly anchor event prep: Emma
    

- Micro=launch anniversary, concert, new arrival
    
- Anchor=in store following specific format
    

- Keep "Staff Picks" / "New Arrivals" areas fresh: Emma & Tre
    

  

Every month

- Create report on performance for month: Tammy 
    

- Total revenue
    
- Top inventory movement
    
- Dead inventory
    

- Send newsletter to subscribers: Emma
    

## Inventory Strategy 

Reorder rules

- If Sales ≥ 3 and On Hand ≤ 1 → reorder 2
    
- If On Hand = 0 and Sales ≥ 2 → reorder 2
    
- If Price > $60 → max reorder 1 unless it has sold 2+
    

  

Monthly dead stock reset

- Items aged 60+ days and low sell-through → move to feature / better placement
    
- Items aged 90+ days and 0 sales → markdown / bundle / remove from prime space
    

  

Inventory budget (until data says otherwise)

- 70% proven sellers + staples
    
- 20% special orders
    
- 10% curated discovery (breadth over depth)
    

## Marketing Strategy

Google Business Profile (highest leverage)

- Post weekly update (new arrivals / event / hours)
    
- Add fresh photos weekly
    
- Respond to every review within 24–48 hrs
    

  

Social (traffic-driving content)

- 3 posts/week:
    

- 1 reel: arrivals flip / "3 you need"
    
- 1 story: staff pick w/ quick narrative
    
- 1 post: event promo / community highlight
    

Local awareness accelerators

- 2 partnerships per quarter (restaurants, barber, coffee, brewery)
    
- Quarterly local PR push (paper / blogs / community pages)
    
- TNT Posters//flyers in community
    

## Events Strategy 

Monthly anchor event (choose one format and repeat)

- Format chosen: 
    
- Next 3 dates scheduled:
    

- Date 1: ___________
    
- Date 2: ___________
    
- Date 3: ___________
    

Weekly micro-event (low lift)

- Release anniversary 
    
- Local concerts
    
- New releases
    

## Best Of Edmonds Strategy (Win plan)

All-year groundwork

- Review volume increases steadily (not bursty)
    
- Community presence is visible monthly (events + partnerships)
    
- Customers know TNT is family-owned, local, curated, helpful
    

When nomination/voting opens (go hard)

- Pinned social posts
    
- In-store signage at checkout
    
- Staff ask every customer directly
    

  
**