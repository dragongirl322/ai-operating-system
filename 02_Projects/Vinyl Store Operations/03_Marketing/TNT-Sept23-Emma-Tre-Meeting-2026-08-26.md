---
title: Year Two (Sept 24) — Emma and Tre meeting
date: 2026-08-26 5:00pm
owner: Tam
status: held 2026-08-26; decisions in TNT-Sept23-Year-Two-Decisions.md
---

# Year Two meeting with Emma and Tre (opening is Sept 24)
Wednesday Aug 26, 5:00pm. Separate from the 3pm Shopify hardware block.

Purpose: hear their ideas first, then lock three decisions so we can market a real day instead of a maybe.

Line: **Year Two. Spinnin' Your Groove.**
Day: Thursday Sept 24, 11–7 (Tre's date; not Sept 23). Customer appreciation. Not a concert, not a bar.

## Already locked (don't reopen unless someone has a hard no)

- Date and hours: Thu Sept 24, 11–7
- Shape: all-day customer appreciation
- No live music, no alcohol (unless Tam changes that later)
- Campaign line stays Year Two. Spinnin' Your Groove.
- Promo does not start until the week of Sept 8
- How we talk: come in, take your time, put a record on, find something you love. Do not use "drop the needle," "needle down," or "dig the bins."

## How to run the meeting

1. Thank them. Year one happened because they ran the floor and the stock.
2. Ask: what would make Sept 24 feel like TNT, for customers and for you two? Capture everything before pitching.
3. Then the three decisions below.
4. Walk out with owners and dates, not a vibe.

## Decision 1 — Approve or adjust the marketing schedule

Draft (Groove). Social announces. The store is the event. Email only if Groove Society / Shopify Email is actually sending by then.

| When | What | Who |
|---|---|---|
| After this meeting / early Sept | Partner asks (café or plaza coupon or pastry) | Tam (Groove drafts) |
| Mon Sept 8 | Save-the-date IG/FB, Google post, in-store sign | Groove drafts, Tam posts |
| Mon Sept 15 | What-to-expect (hours, Year Two line, whatever we lock tonight). One shop photo. | Groove drafts; Emma or Tre photo |
| Mon Sept 21 | Press tip to My Edmonds News / Beacon + 2 photos | Groove drafts, Tam sends |
| Tue Sept 22 | Tomorrow reminder IG/FB/Google | Groove drafts, Tam posts |
| Thu Sept 24 | Stories + Google. Host the day. | Emma floor, Tre stack, Tam thanks |
| Thu Sept 24 | Thank-you recap | Groove drafts, Tam posts |

Ask them: anything missing, too much, or in the wrong order? If they want a photo or a staff-pick post, put a name on it tonight.

## Decision 2 — One day, or a couple of micro events around it

Options to put on the table (all still no live music / no alcohol):

- **A. Day-of only.** Sept 24 is the whole thing. Simplest. Matches what we locked Aug 24, date corrected to the 24th.
- **B. Day-of plus a small surround.** Examples that fit TNT: a listening hour the Saturday before, a staff-favorites week on the floor, or a "bring a friend" weekend after. Not extra production. Not a second party.
- **C. Something they invent.** If Emma or Tre has a better surround, hear it. Kill anything that needs a stage, a bar, or a special buy Tre cannot cover from stock.

Decide: A, B, or C. If B or C, name the extra beat and who runs it.

## Decision 3 — What we actually do in the store on Sept 24

Tam proposes a **prize wheel** (partner coupons plus one TNT prize: sticker pack or a cleaning credit). That is a proposal, not a lock until they react.

Also on the table:

- Conversation-level music, easy browsing, thank people for year one
- Tre: one Year Two staff-favorites / giftable stack from existing stock. No special buy unless Tam says so
- Emma: host the floor. If the wheel is in, she runs it. Groove Society ask at checkout either way
- Tam: partners, budget, photos, thank-yous

If they hate the wheel, replace it with something they will actually run (a simple drawing, a cleaning credit punch card, a "thank you" sticker, nothing). Empty ritual is worse than no wheel.

If the wheel stays, lock tonight:

- Who can spin (every customer? purchase required?)
- What is on the wheel (TNT prize + partner coupons)
- Who fetches prizes when it is busy
- Where it sits so it does not block bins or the register

## Gaps to cover (easy to skip, costly later)

**Partners.** Wheel coupons need 1–2 plaza or café names. Ask Emma and Tre who already likes us. Tam still owns the ask. Skip brewery unless Tam lifts the alcohol rule.

**Staffing.** Can Emma and Tre cover 11–7, or do we need a peak-hour extra body? Who takes lunch?

**Budget.** Plan-of-record sketch is about $50 prizes and about $40 partner hospitality, inside the ~$250/mo marketing cap. Tam confirms the number. Tre does not spend inventory money on this unless Tam says.

**Photos.** Need one honest floor/staff photo for the Sept 15 post, plus two for the press tip. Who shoots, by when?

**Groove Society.** Emma's ask at checkout that day. One sentence they are willing to say. Do not invent a list if Shopify capture is not live yet.

**Google reviews.** Light ask to happy customers that day (Emma). Not a speech.

**Busy vs quiet.** If it is slammed: wheel pauses, stack stays full, checkout first. If it is quiet: still thank people, still take the photo.

## Walk out with these filled in

- [ ] Marketing schedule: approved as-is / adjusted (note the change)
- [ ] Shape: day-of only / day-of plus micro events (name them)
- [ ] In-store: wheel yes/no. If no, what instead
- [ ] Wheel rules, if yes (who spins, prizes, where, who runs it)
- [ ] Tre's stack: yes, from stock, no special buy unless Tam says
- [ ] Partner names to ask (1–2)
- [ ] Staffing 11–7
- [ ] Photo owner and deadline
- [ ] Groove Society sentence for Emma
- [ ] Who tells Groove the decisions after this meeting (Tam)

Tam owns partners and money. Emma owns the floor. Tre owns the stack. Groove drafts promo after this meeting, does not invent a new day-shape.


## Decisions from the meeting (Tam, after 5pm)

See `TNT-Sept23-Year-Two-Decisions.md`. Short version:

- Shape: day-of plus a raffle that starts Sept 1
- Free item bin with any purchase (day-of)
- SACDs moved; may sell wholesale
- Vouchers with Five Corners Plaza
- Raffle: full-price purchase enters; winner gets red turntable or a boombox, plus two floor albums of their choice
- Prize wheel not named as the live mechanic; raffle is

