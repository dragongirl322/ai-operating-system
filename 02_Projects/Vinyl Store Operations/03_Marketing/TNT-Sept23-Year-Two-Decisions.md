---
title: Sept 24 Year Two — locked after Emma/Tre
updated: 2026-09-01
status: locked from Tam 8/26 5pm
campaign: Year Two. Spinnin' Your Groove.
---

# Sept 24 — what we are actually doing

Locked after Tam talked with Emma and Tre (Aug 26). Campaign line stays **Year Two. Spinnin' Your Groove.** Day is Thu Sept 24, 11–7 (Tre's date; not the 23rd), customer appreciation. No live music, no alcohol.

## In the store on Sept 24

- **Free item with purchase.** Anyone who buys something gets a free item from a free bin.
- **SACDs.** Stay where they are. Not moving for Year Two. Not selling at wholesale that day. (Tre decided 8/31 night, Tam agreed 9/1.)
- **Five Corners Plaza vouchers.** Partner piece. Plaza businesses on the thank-you / voucher.
- **Tre:** still one Year Two staff-favorites / giftable stack from stock. No special buy unless Tam says.
- **Emma:** host the floor, Groove Society ask at checkout, run whatever prize mechanic is live that day.

Prize wheel was Tam’s proposal going into the meeting. The named prize mechanic coming out is the raffle (below). Do not rebuild a wheel unless Tam puts it back.

## Surround: raffle starts Sept 1

Not day-of only. The extra beat is a raffle that starts **Tuesday Sept 1**.

- **Who enters:** anyone who buys something at **full price**
- **Prize (one winner):** the red turntable **or** one of the boom boxes, **plus** two albums on the floor of their choice
- **Tickets:** buy an album = 1 ticket. Emma asks them to post the purchase on social and tag TNT; that is 5 tickets.
- **Emma's collector:** web app, live Tuesday Sept 1. Tam (or Groove if she says go) builds it Friday Aug 28.
- **Signage:** Dayna looks into the sandwich board Monday Aug 31.
- **Assume until Tam says otherwise:** entries run Sept 1 through close on Sept 24; draw at the store on Sept 24

This pulls the marketing start earlier than the Sept 8 save-the-date. In-store raffle sign and a short IG/FB/Google note need to exist by Sept 1.

## Marketing (adjusted)

| When | What | Who |
|---|---|---|
| By Tue Sept 1 | Raffle live in-store. Short post: full-price buy = raffle entry, prize is red turntable or boombox + two albums. | Groove drafts, Tam posts; Emma/Tre run the entry |
| After this meeting / early Sept | Five Corners Plaza voucher ask | Tam (Groove drafts) |
| Mon Sept 8 | Save-the-date for Sept 24 (free bin, plaza vouchers, SACDs if we want them in the copy) | Groove drafts, Tam posts |
| Mon Sept 15 | What-to-expect. One shop photo. | Groove drafts; Emma or Tre photo |
| Mon Sept 21 | Press tip to My Edmonds News / Beacon + 2 photos | Groove drafts, Tam sends |
| Wed Sept 23 | Tomorrow reminder | Groove drafts, Tam posts |
| Thu Sept 24 | Host the day. Draw the raffle. Stories + Google. | Emma floor, Tre stack, Tam thanks |
| Fri Sept 25 | Thank-you recap | Groove drafts, Tam posts |

Email only if Groove Society / Shopify Email is actually sending by then.

## Still open (does not block Sept 1)

- [done 9/1] SACDs stay where they are. Not wholesale.
- What goes in the free bin (and who restocks it)
- Which plaza shops are on the voucher, and the ask copy
- Full-price definition at the register (sale / used / already-discounted)
- Who draws the raffle and how Emma logs entries
- Staffing 11–7 and photos

## Owners

Tam: partners, budget, voucher ask, approvals.
Emma: floor, raffle entries, free bin, Groove Society ask.
Tre: Year Two stack from stock, restock. SACDs stay put.
Groove: promo drafts from this file, press tip, voucher-ask draft. Does not invent a new day-shape.


## After opening day (locked Tam 8/27)

1. **Halloween.** Plan right after Sept 24 (start Fri Sept 25). One bigger event plus micro-events through much of October.
2. **Holiday inventory.** Order mid-October (target Mon Oct 12). Vinyl Scout remains weekly reorder truth; this is Tam-approved holiday/giftable extras. Tre places the order.
3. **Thanksgiving through December.** Plan off the Sept 25 sketch so the October order matches the events. Then execute. No live music / no alcohol unless Tam changes that.
